...
- **NATIVE PORT: ALIGN NOTIFICATION SERVICE, COMMAND SCHEMAS, & TCP/TOKEN AUTH (2026-06-28):**
  Aligned the native Rust daemon's macOS notification service monitor (`macos_notifications.rs`), routing, and command responses with the ground-truth Python daemon. Ported the headless TCP server listener and token authentication features.
  ...
  **Changes shipped:**
  ...
  - `tests/test_rust_daemon_parity.py`: Shipped a new Python integration test suite verifying the socket response shapes of the compiled Rust daemon subprocess using Python `DaemonClient` over both Unix and TCP transports.

  **Tests:** Rust 51 passed; Python 1710 passed, 87 skipped. (Verified with new integration tests).
