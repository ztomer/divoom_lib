...
class System:
    ...
    async def get_work_mode(self):
        ...
        response = await self.communicator.send_command_and_wait_for_response("get work mode")
        if response:
    ...
    async def set_channel(self, channel_id: int):
        ...
        channel_id:
            ...
            0x06: Scoreboard
        """
        if not (0x00 <= channel_id <= 0x06):
            self.logger.error(f"Invalid channel ID: {channel_id}. Must be between 0x00 and 0x06.")
            return False

        self.logger.info(f"Switching to channel: {channel_id} (0x45)...")
        args = [channel_id]
        return await self.communicator.send_command("set channel", args)
    ...
    async def send_sd_status(self, status: int):
        ...
        args = [status]
        return await self.communicator.send_command("send sd status", args)
    ...
    async def set_boot_gif(self, on_off: int, total_length: int, gif_id: int, data: list):
        ...
        args.extend(data)
        return await self.communicator.send_command("set boot gif", args)

    async def get_device_temp(self):
        """Get the device's temperature (0x59)."""
        self.logger.info("Getting device temperature (0x59)....")
        response = await self.communicator.send_command_and_wait_for_response("get device temp")
    ...
    async def send_net_temp(self, year: int, month: int, day: int, hour: int, minute: int, num: int, temp_data: list):
        ...
        return await self.communicator.send_command("send net temp", args)
    ...
    async def send_net_temp_disp(self, display_modes: list, time_minutes: int):
        ...
        return await self.communicator.send_command("send net temp disp", args)
    ...
    async def get_net_temp_disp(self):
        ...
        response = await self.communicator.send_command_and_wait_for_response("get net temp disp")
    ...
    async def set_device_name(self, name: str):
        ...
        return await self.communicator.send_command("set device name", args)

    async def get_device_name(self):
        """Obtain the Bluetooth device name (0x76)."""
        self.logger.info("Getting device name (0x76)...")
        response = await self.communicator.send_command_and_wait_for_response("get device name")
    ...
    async def set_low_power_switch(self, on_off: int):
        ...
        args = [on_off]
        return await self.communicator.send_command("set low power switch", args)
    ...
    async def get_low_power_switch(self):
        ...
        response = await self.communicator.send_command_and_wait_for_response("get low power switch")
        if response and len(response) >= 1:
    ...
    async def set_hour_type(self, hour_type: int):
        ...
        args = [hour_type]
        return await self.communicator.send_command("set time type", args)
    ...
    async def set_song_display_control(self, control: int):
        ...
        args = [control]
        return await self.communicator.send_command("set song dis ctrl", args)
    ...
    async def set_bluetooth_password(self, control: int, password: str = None):
        ...
        return await self.communicator.send_command("set blue password", args)
    ...
    async def set_power_on_voice_volume(self, control: int, volume: int = None):
        ...
        if control == 1 and volume is not None:
            if not (0 <= volume <= 100):
                ...
                return False
            args.append(volume)
        return await self.communicator.send_command("set poweron voice vol", args)
    ...
    async def set_power_on_channel(self, control: int, channel_id: int = None):
        ...
        return await self.communicator.send_command("set poweron channel", args)
    ...
    async def set_auto_power_off(self, minutes: int):
        ...
        return await self.communicator.send_command("set auto power off", list(args))
    ...
    async def get_auto_power_off(self):
        ...
        response = await self.communicator.send_command_and_wait_for_response("get auto power off")
        if response and len(response) >= 2:
    ...
    async def set_sound_control(self, enable: int):
        ...
        args = [enable]
        return await self.communicator.send_command("set sound ctrl", args)
    ...
    async def get_sound_control(self):
        ...
        response = await self.communicator.send_command_and_wait_for_response("get sound ctrl")
        if response and len(response) >= 1:
