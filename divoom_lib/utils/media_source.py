...
_FEISHIN_CREDS_CACHE = None  # (url, auth_query, username)
_FEISHIN_CREDS_AT = 0.0      # monotonic timestamp of last credential scan


def _feishin_creds() -> tuple | None:
    """Extract Navidrome server credentials from Feishin's Local Storage."""
...
def _feishin_is_running() -> bool:
    """Quick check whether Feishin.app is running."""
    try:
...
def get_feishin_playing_track() -> dict | None:
    """
    Query Feishin's Navidrome server via the Subsonic API for currently-
    playing track info. Returns the same dict shape as the other sources.
    """
