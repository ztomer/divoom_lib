...
async def find_and_connect_timoo(probe_lightning: bool = False, skip_channel_switch: bool = False, replay_legacy: bool = False):
        ...
        try:
            async with BleakClient(timoo_device.address) as client:
                if client.is_connected:
                    ...
                    # Quick verification mode: replay the exact legacy iOS-LE inner payload
                    # observed in probe.txt across discovered write characteristics. This
                    # attempts the iOS-LE outer framing with command 0x45 and inner payload
                    # starting with 0x01 (mode) followed by the long legacy fields.
                    if replay_legacy:
                        legacy_payload = [0x01, 0xFF, 0xFF, 0xFF,
                                          0x64, 0x00, 0x01, 0x00, 0x00, 0x00]
                        print(
                            "--replay-legacy enabled: will replay legacy iOS-LE 0x45 payload across write characteristics")
                        for char_uuid in write_characteristics:
                            print(
                                f"[replay-legacy] Testing characteristic {char_uuid} -> sending legacy iOS-LE packet")
                            try:
                                ok, ack_sender, raw = await send_command_and_wait_for_ack(client, char_uuid, 0x45, legacy_payload, use_ios_le=True, timeout=4.0)
                                if ok:
                                    print(
                                        f"[replay-legacy] Success on {char_uuid} (ACK from {ack_sender}), response={raw.hex()}")
                                    try:
                                        save_working_mapping(
                                            client, char_uuid, str(ack_sender))
                                    except Exception as e:
                                        print(
                                            f"[replay-legacy] Failed to save mapping: {e}")
                                    try:
                                        # Persist only the inner payload (legacy_payload).
                                        # construct_ios_le_packet already includes the outer
                                        # command byte (0x45) and packet number when called
                                        # with command_code=0x45, so saving legacy_payload
                                        # allows correct replay later.
                                        save_last_successful_payload(
                                            char_uuid, legacy_payload, use_ios_le=True)
                                    except Exception:
                                        pass
                                else:
                                    print(
                                        f"[replay-legacy] No ACK on {char_uuid}")
                            except Exception as e:
                                print(
                                    f"[replay-legacy] Error testing {char_uuid}: {e}")

                        print("--replay-legacy completed; exiting test run.")
                        return
                        ...
                        try:
                            ...
                            if saved and saved.get("last_successful_payload"):
                                ...
                                if saved_payload:
                                    ...
                                    ok_saved, ack_saved, _raw_saved = await send_command_and_wait_for_ack(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skip-channel-switch", action="store_true",
                        help="Do not send channel/power-on commands; try saved payloads before switching channels")
    parser.add_argument("--replay-legacy", action="store_true",
                        help="Quickly replay the legacy iOS-LE 0x45 payload (from probe) across discovered write characteristics and report which one ACKed")
    args = parser.parse_args()
    asyncio.run(find_and_connect_timoo(probe_lightning=args.probe_lightning,
                skip_channel_switch=args.skip_channel_switch, replay_legacy=args.replay_legacy))
