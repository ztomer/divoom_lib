"""R17 P5 — GUI-side daemon bridge: the device proxy dispatched against a real
in-process daemon (temp socket, fake device).

Split out of tests/test_daemon_bridge.py: every test here is built on the
``live_daemon`` fixture, which instantiates the archived
divoom_daemon.daemon.DivoomDaemon server. The auto-spawn/TCC-disclaim/proxy-
cache tests that don't need a live daemon stayed in tests/test_daemon_bridge.py.
"""
import asyncio
import os
import sys
