...
def mock_divoom_instance():
    ...
    with patch('divoom_lib.protocol.DivoomProtocol.send_command_and_wait_for_response', new_callable=AsyncMock) as mock_send_command_and_wait, \
        ...
        divoom.protocol.client = AsyncMock()
        divoom.protocol.client.is_connected = True
        divoom.protocol.WRITE_CHARACTERISTIC_UUID = "mock_write_char_uuid"
        divoom.protocol.NOTIFY_CHARACTERISTIC_UUID = "mock_notify_char_uuid"
        divoom.protocol.use_ios_le_protocol = False
        divoom.protocol.escapePayload = False

        divoom.protocol.mock_send_command_and_wait_for_response = mock_send_command_and_wait
        divoom.protocol.mock_send_command = mock_send_command

        divoom.protocol.mock_send_command_and_wait_for_response.return_value = b'\x01\x00\x00\x00\x00\x02'
        divoom.protocol.mock_send_command.return_value = True
...
async def test_divoom_init(mock_divoom_instance):
    ...
    assert divoom.protocol.mac == "AA:BB:CC:DD:EE:FF"
    assert divoom.protocol.device_name == "MockDevice"
...
async def test_framing_context(mock_divoom_instance):
    ...
    divoom = mock_divoom_instance
    original_use_ios = divoom.protocol.use_ios_le_protocol
    original_escape = divoom.protocol.escapePayload

    async with divoom.protocol._framing_context(use_ios=True, escape=True):
        assert divoom.protocol.use_ios_le_protocol is True
        assert divoom.protocol.escapePayload is True

    assert divoom.protocol.use_ios_le_protocol == original_use_ios
    assert divoom.protocol.escapePayload == original_escape
...
async def test_try_send_command_with_framing(mock_divoom_instance):
    ...
    divoom.protocol.mock_send_command_and_wait_for_response.reset_mock()
    await divoom._try_send_command_with_framing(command_id, payload, timeout=3, use_ios=True, escape=True)
    assert divoom.protocol.use_ios_le_protocol is True
    assert divoom.protocol.escapePayload is True
    divoom.protocol.mock_send_command_and_wait_for_response.assert_called_once_with(command_id, payload, timeout=3)

    divoom.protocol.mock_send_command_and_wait_for_response.reset_mock()
    await divoom._try_send_command_with_framing(command_id, payload, timeout=3, use_ios=False, escape=False)
    assert divoom.protocol.use_ios_le_protocol is False
    assert divoom.protocol.escapePayload is False
    divoom.protocol.mock_send_command_and_wait_for_response.assert_called_once_with(command_id, payload, timeout=3)
...
async def test_send_diagnostic_payload_ios_success(mock_divoom_instance):
    ...
    divoom.protocol.mock_send_command_and_wait_for_response.side_effect = [
        None,
...
async def test_send_diagnostic_payload_failure(mock_divoom_instance):
    ...
    divoom.protocol.mock_send_command_and_wait_for_response.return_value = None
...
async def test_probe_write_characteristics_fallback_spp_success(mock_divoom_instance):
    ...
    assert divoom.protocol.mock_send_command.call_count >= 2
    assert divoom.protocol.mock_send_command_and_wait_for_response.call_count >= 2
...
async def test_probe_write_characteristics_fallback_ios_success(mock_divoom_instance):
    ...
    assert result is None
    assert divoom.protocol.mock_send_command.call_count >= 2
    assert divoom.protocol.mock_send_command_and_wait_for_response.call_count >= 3
...
async def test_probe_write_characteristics_fallback_failure(mock_divoom_instance):
    ...
    write_chars = [MagicMock(uuid="char1_uuid")]
    divoom.protocol.mock_send_command_and_wait_for_response.return_value = None
    divoom.protocol.mock_send_command.return_value = False
