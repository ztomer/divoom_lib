...
Verifies the 0x8B 3-phase streamer uses the proven BLE-safe semantics that the
monthly-best daemon uses (and the naive show_image loop did NOT): a sequential
chunk-INDEX offset_id (0,1,2,…), 200-byte chunks, start + N data + terminate.
"""
...
async def test_stream_phases_and_chunk_index_offsets():
    ...
    blob = bytes(range(256)) * 2  # 512 bytes → ceil(512/200) = 3 data chunks
