...
async def debug_light_settings(device_name_substring: str = "Tivoo"):
    ...
    found_devices = await discover_divoom_devices(device_name_substring=device_name_substring, logger=logger)
    ...
    # Discover characteristics for the target device
    write_uuid, notify_uuid, read_uuid = await discover_characteristics(target_device.address, logger)

    if not all([write_uuid, notify_uuid, read_uuid]):
        print_err(f"Could not discover all required characteristics for {target_device.name}. Exiting.")
        return

    divoom_instance = None
    try:
        divoom_instance = Divoom(
            mac=target_device.address,
            logger=logger,
        ...
        print_info(f"Testing Method 1: divoom_api.display.show_light (Red - FF0000)")
        ...
        # --- Method 2: divoom_api.commands.brightness.BrightnessCommand.set_brightness (Blue) ---
        # First, ensure the light channel is still active (it should be from Method 1)
        # Then, change brightness and color using a different command
        print_info(f"Testing Method 2: divoom_api.commands.brightness.BrightnessCommand.set_brightness (Blue - 0000FF)")
        # Set the light to blue using show_light first, then adjust brightness
        await divoom_instance.display.show_light(color="0000FF", brightness=50)
        await divoom_instance.brightness_command.set_brightness(100) # Max brightness
        print_ok(f"Method 2: Light set to Blue, then brightness to 100. Waiting 5 seconds...")
        await asyncio.sleep(5)

        # --- Method 3: divoom_api.display.show_effects (Green) ---
        print_info(f"Testing Method 3: divoom_api.display.show_effects (Green - 00FF00)")
